...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">{{ __('Create New Landing Page Element') }}</div>

                <div class="card-body">
                    <form method="POST" action="{{ route('admin.landing-page-elements.store') }}">
                        @csrf
                        ...
                        <div class="mb-3">
                            ...
                            <select id="element_type" class="form-select @error('element_type') is-invalid @enderror" name="element_type" required>
                                ...
                                <option value="heading" {{ old('element_type') == 'heading' ? 'selected' : '' }}>Heading</option>
                                <option value="paragraph" {{ old('element_type') == 'paragraph' ? 'selected' : '' }}>Paragraph</option>
                                <option value="image" {{ old('element_type') == 'image' ? 'selected' : '' }}>Image</option>
                                <option value="button" {{ old('element_type') == 'button' ? 'selected' : '' }}>Button</option>
                                <option value="icon" {{ old('element_type') == 'icon' ? 'selected' : '' }}>Icon</option>
                            </select>
                            @error('element_type')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                        ...
                        <div class="mb-3">
                            ...
                            <input id="position" type="number" class="form-control @error('position') is-invalid @enderror" name="position" value="{{ old('position') ?? 0 }}" required>
                            @error('position')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
